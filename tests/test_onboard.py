"""Tests for onboard.py (pure helpers only, no interaction/network)."""
import unittest
...
class TestKeyState(unittest.TestCase):
    def test_states(self):
        self.assertEqual(ob.key_state("", "GROQ_API_KEY"), "empty")
        self.assertEqual(ob.key_state("gsk-change-me", "GROQ_API_KEY"), "placeholder")
...
class TestEnvEditing(unittest.TestCase):
    def test_get_set_roundtrip(self):
        lines = ["# comment", "FOO=old", "", "BAR=stays"]
        self.assertEqual(ob.get_value(lines, "FOO"), "old")
        ob.set_value(lines, "FOO", "new")
        self.assertEqual(ob.get_value(lines, "FOO"), "new")
        # comments/other lines untouched
        self.assertEqual(lines[0], "# comment")
        self.assertEqual(lines[3], "BAR=stays")

    def test_set_appends_missing_key(self):
        lines = ["FOO=1"]
        ob.set_value(lines, "NEW", "value")
        self.assertIn("NEW=value", lines)
...
class TestMask(unittest.TestCase):
    def test_mask_never_reveals_full_value(self):
        ...
        self.assertEqual(ob.mask(""), "(empty)")


class TestProviderKeyTable(unittest.TestCase):
    def test_covers_all_provider_env_vars(self):
        """Every env var defined in providers_config must show up in the
        onboarding flow (otherwise it can't be entered interactively)."""
